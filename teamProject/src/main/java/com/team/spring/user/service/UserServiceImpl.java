...
import com.team.spring.user.dao.UserCommentDao;
import com.team.spring.user.dao.UserDao;
...
public class UserServiceImpl implements UserService{
	...
	public void getList(HttpServletRequest request) {
		...
		//검색과 관련된 정보를 userDto 객체에 선택적으로 담는다.
		...
		//전체 row 의 갯수를 읽어온다.
		int totalRow=userDao.getCount(dto);
		//전체 페이지의 갯수 구하기
		int totalPageCount=
		...
		// startRowNum 과 endRowNum 을 userDto 객체에 담고 
		dto.setStartRowNum(startRowNum);
		dto.setEndRowNum(endRowNum);
		
		// startRowNum 과 endRowNum 에 해당하는 카페글 목록을 select 해 온다.
		List<UserDto> list=userDao.getList(dto);
	...
	public void getDetail(HttpServletRequest request) {
		...
		//글 조회수 올리기
		userDao.addViewCount(num);
		//request 에 글정보를 담고
		request.setAttribute("dto", resultDto);
		//댓글 목록을 얻어온다. (자세히 보기할 글번호가 댓글의 ref_group 번호)
		List<UserCommentDto> commentList=userCommentDao.getList(num);
	...
	public void getUpdateData(ModelAndView mView, int num) {
		...
		UserDto dto=userDao.getData(num);
	...
	public void saveComment(HttpServletRequest request) {
		...
		int seq=userCommentDao.getSequence();
		//댓글 정보를 userCommentDto 객체에 담는다.
		...
		//댓글 정보를 DB 에 저장한다.
		userCommentDao.insert(dto);
	}

	@Override
	public void updateComment(UserCommentDto dto) {
		//userCommentDao 객체를 이용해서 댓글 내용을 update 한다. 
		userCommentDao.update(dto);
	}
